...
=head1 SYNOPSIS
  ...
  # Deque container...
  my $d = deque(qw(first second third fourth));
  ...
  print "Element 'second' was ", 
    find_if($d->begin(), $d->end(), bind1st(equal_to(), 'second')) 
      ? 'found' : 'not found', "\n";

  # Algorithms -- count_if()
  print "Number of elements matching /o/ = ",
    count_if($d->begin(), $d->end(), bind2nd(matches(), 'o')),
	"\n"; # prints '2' -- matches 'second' and 'fourth'

  # Algorithms -- transform()
  transform($d->begin(), $d->end(), $d2->begin(), ptr_fun('ucfirst'));
  transform($d->begin(), $d->end(), $d2->begin(), $d3->begin(), ptr_fun_binary('::mybfun'));
  ...
  my $v = queue(qw(first second third fourth fifth));
  ...
  # Vector container...
  my $v = vector(qw(first second third fourth fifth));
  ...
  my $d2 = deque(qw(sixth seventh eight));
  ...
  # DataMembers -- Class builder helper...
  {
    ...
    sub BEGIN 
    { 
      Class::STL::DataMembers->new(qw( attrib1 attrib2 ), # simple data members
        Class::STL::DataMembers::Attributes->new(  # complex data members
    ...
    sub new
    {
        ...
        bless($self, $class);
        $self->members_init(@_); # initialise any named arguments.
        return $self;
    }
  }
  ...
  # MakeFind -- Class builder helper...
  # MakeFind is only available to classes derived from Class::STL::Containers::Abstract.
  {
    package MyContainerClass;
    use base qw(Class::STL::Containers::Vector);
    sub begin
    {
      Class::STL::DataMembers->new(qw( name value ));
      Class::STL::Containers::MakeFind(qw( name )); # Create find() function based on 'name' members.
    }
    sub new
    {
        my $self = shift;
  ...
  my $cl = MyContainerClass->new();
  $cl->push_back($cl->factory(name => 'red', value => 100));
  $cl->push_back($cl->factory(name => 'blue', value => 102));
  $cl->push_back($cl->factory(name => 'green', value => 104));
  $cl->push_back($cl->factory(name => 'orange', value => 107));
  print "Green value=", $cl->find('green')->value(), "\n"; # 'Green value=104';
    # find element where name eq 'green';
...
=item The I<end()> Function

This function will return a newly constructed iterator object which will point to the I<last> element within the container, unlike the C++/STL equivalent which points to I<after> the last element. 
...
=head1 F<CLASS Class::STL::DataMembers>

This I<helper> class can be used to generate code for various basic class functions. An object of this class should be constructed (and destroyed) in the F<BEGIN> subroutine for the target class. Construction is done via its I<new> function. This function requires a list consisting of target data member names, or I<Class::STL::DataMembers::Attributes> objects. When using I<DataMembers> B<ALL> data members should be included in order for the generated I<clone> and I<swap> functions to function correctly. 


A debug version of I<new>, I<new_debug> can be used to display the generated code. The following target member functions will be generated and made available to the class:
...
=item Class::STL::DataMembers::Attributes
...
The I<new> function constructs an object for this class and returns a blessed reference to this object. All forms accept an optional I<hash> containing any of the following named arguments: I<data_type>. The I<data_type> defines the class type of element objects that the container will hold.
...
F<remove_if>, F<find_if>, F<for_each>, F<transform>, F<count_if>, F<find>, F<count>, F<copy>, F<copy_backward>, F<remove>, F<remove_copy>, F<remove_copy_if>, F<replace>, F<replace_if>, F<replace_copy>, F<replace_copy_if>, 
...
I<void for_each ( iterator-start, iterator-finish, unary-function-object );>


The I<for_each> function will traverse the container starting from I<iterator-start> and ending at I<iterator-finish> and execute the I<unary-function-object> with the element passed in as the argument.  
...
I<iterator-ref find_if ( iterator-start, iterator-finish, unary-function-object );>


The I<find_if> function will traverse the container starting from I<iterator-start> and ending at I<iterator-finish> and return an I<iterator> pointing to the first element that evaluate to true by the I<unary-function-object>. If no elements evaluates to true then 'o' is returned.
...
I<void remove_copy_if ( iterator-start, iterator-finish, iterator-result, unary-function-object );>
...
I<void replace_if ( iterator-start, iterator-finish, unary-function-object, new-element-ref );>
...
I<void replace_copy_if ( iterator-start, iterator-finish, iterator-result, unary-function-object, new-element-ref );>
...
F<iteratror>, F<bidirectional_iteratror>, F<reverse_iteratror>, F<forward_iteratror>, F<++>, F<-->, F<==>, F<!=>, E<gt>F<=>, E<lt>F<=>.
