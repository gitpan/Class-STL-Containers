...
=head1 SYNOPSIS
  ...
  # Inserters
  my $three2one = list(qw(3 2 1));
  my $four2six = list(qw(4 5 6));
  my $seven2nine = list(qw(7 8 9));
  my $result = list();
  copy($three2one->begin(), $three2one->end(), front_inserter($result));
  copy($seven2nine->begin(), $seven2nine->end(), back_inserter($result));
  my $iseven = find($result->begin(), $result->end(), 7);
  copy($four2six->begin(), $four2six->end(), inserter($result, $iseven));
  ...
  # MakeFind is only available to classes derived from Class::STL::Containers::Abstract.
  {
    ...
    sub begin
    {
      ...
      Class::STL::Containers::MakeFind->new(qw( name )); # Create find() function based on 'name' members.
    }
    sub new
    {
        my $self = shift;
